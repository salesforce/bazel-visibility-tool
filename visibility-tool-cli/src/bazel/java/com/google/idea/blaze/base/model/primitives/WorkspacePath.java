 ...
 *
 * <p>
 * A {@link WorkspacePath} is *not* necessarily a valid package name/path. The primary reason is because it could
 * represent a file and files don't have to follow the same conventions as package names.
 */
@Immutable
public class WorkspacePath implements Serializable {
    ...
    /**
     * @param relativePath
     *            relative path that must use the Blaze specific separator char to separate path components
     * @throws IllegalArgumentException
     *             if the path is invalid
     */
    public WorkspacePath(String relativePath) {
        var error = validate(relativePath);
        if (error != null) {
    ...
    public WorkspacePath(WorkspacePath parentPath, String childPath) {
        this(parentPath.isWorkspaceRoot() ? childPath
    ...
    public boolean equals(Object o) {
        ...
        var that = (WorkspacePath) o;
        return relativePath.equals(that.relativePath);
    }

    /**
     * Returns the workspace path of this path's parent directory. Returns null if this is the workspace root.
     */
    @Nullable
    public WorkspacePath getParent() {
        ...
        var parentPath = lastSeparatorIndex == -1 ? "" : relativePath.substring(0, lastSeparatorIndex);
